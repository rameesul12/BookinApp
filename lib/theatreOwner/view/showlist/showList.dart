...
class ShowListPage extends StatelessWidget {
 const ShowListPage({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body:Consumer<AddShowProvider>(
        builder: (context,data,child) {
          return Column(
            children: [
              sizedH60,
            const  Text('shows',style: TextStyle(color: textwhite,fontWeight: FontWeight.bold,fontSize: 30,),),
             if (data.shows.isEmpty) const Expanded(child: Center(child: Text("there Is No show Added",style: TextStyle(color: textwhite),),)) else Expanded(
                  ...
                  sizedH10,
                   ...
                   mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [
                      ...
                      sizedW20,
                          Expanded(child: Text("${provider.shows[index].screen}",style: TextStyle(color: colorTextwhite),)),
                    ],
                  ),
                  Row(
                   mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      ...
                      sizedW20,
                          Expanded(child: Text(provider.shows[index].movieName,style: const TextStyle(color: colorTextwhite),)),
                    ],
                  ),
                   ...
                   mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [
                     const   Text("Show Time :",style: TextStyle(color: colorTextwhite,fontWeight: FontWeight.bold,fontSize: 18),),
                      sizedW20,
                       Expanded(child:    Text(provider.shows[index].showTime,style: const TextStyle(color: colorTextwhite),)),
                    ],
                  ),
                     Row(
                   mainAxisAlignment: MainAxisAlignment.spaceAround,
                    ...
                    const   Text("Start Date :",style: TextStyle(color: colorTextwhite,fontWeight: FontWeight.bold,fontSize: 18),),
                      sizedW20,
                       Expanded(child:    Text("${provider.shows[index].startDate}".substring(0,10),style: TextStyle(color: colorTextwhite),)),
                    ],
                  ),
                    Row(
                   mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [
                      ...
                      sizedW20,
                       Expanded(child:    Text("${provider.shows[index].endDate}".substring(0,10),style:const TextStyle(color: colorTextwhite),)),
                    ],
                  ),
                  Row(
                   mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [
                      ...
                      sizedW20,
                       Expanded(child:    Text("${provider.shows[index].price}",style:const TextStyle(color: colorTextwhite),)),
                    ],
                  ),
